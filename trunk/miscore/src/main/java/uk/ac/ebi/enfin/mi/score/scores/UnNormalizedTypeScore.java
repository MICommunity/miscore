...
public class UnNormalizedTypeScore extends UnNormalizedCategoryScore {
    ...
    private void setOntologyTypeScores(){
        ...
        defaultOntologyScore.put(categoryScores.getProperty("type.cv1.id"),  new Float(categoryScores.getProperty("type.cv1.unNormalizedScore"))); // cv1 // genetic interaction
        defaultOntologyScore.put(categoryScores.getProperty("type.cv2.id"),  new Float(categoryScores.getProperty("type.cv2.unNormalizedScore"))); // cv2 // colocalization
        defaultOntologyScore.put(categoryScores.getProperty("type.cv3.id"),  new Float(categoryScores.getProperty("type.cv3.unNormalizedScore"))); // cv3 // association
        defaultOntologyScore.put(categoryScores.getProperty("type.cv4.id"),  new Float(categoryScores.getProperty("type.cv4.unNormalizedScore"))); // cv4 // physical association
        defaultOntologyScore.put(categoryScores.getProperty("type.cv5.id"),  new Float(categoryScores.getProperty("type.cv5.unNormalizedScore"))); // cv5 // direct interaction
        /* Ontology terms not present in OLS will be consider null and classify as "unknown" terms */
        defaultOntologyScore.put(categoryScores.getProperty("type.cv6.id"),  new Float(categoryScores.getProperty("type.cv6.unNormalizedScore"))); // cv6 // unknown
