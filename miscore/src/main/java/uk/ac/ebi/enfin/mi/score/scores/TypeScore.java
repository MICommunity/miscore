...
public class TypeScore extends CategoryScore {
     ...
     */
    public TypeScore(List<String> listOfOntologyTerms) {
        this(listOfOntologyTerms,true);
    }
    public TypeScore(List<String> listOfOntologyTerms,boolean useOls) {
        ...
        mapOfTypeTerms.put(categoryScores.getProperty("type.cv3.id"), new HashMap<>());
        mapOfTypeTerms.put(categoryScores.getProperty("type.cv4.id"), new HashMap<>());
        /* No need to look for MI:0403 since it has no children. Just include them in the mappingParentTerms */
        mapOfTypeTerms.put(categoryScores.getProperty("type.cv2.id"), new HashMap<>());
     ...
     */
    public TypeScore(List<String> listOfOntologyTerms, Map<String, Map<String,String>> mapOfTypeTerms) {
        super(listOfOntologyTerms);
        setOntologyTypeScores();
        setOntologyTypeCategories();
        setMappingParentTerms(mapOfTypeTerms);
    ...
    private void setOntologyTypeScores(){
        ...
        defaultOntologyScore.put(categoryScores.getProperty("type.cv1.id"), Float.parseFloat(categoryScores.getProperty("type.cv1.score"))); // cv1 // genetic interaction
        defaultOntologyScore.put(categoryScores.getProperty("type.cv2.id"), Float.parseFloat(categoryScores.getProperty("type.cv2.score"))); // cv2 // colocalization
        defaultOntologyScore.put(categoryScores.getProperty("type.cv3.id"), Float.parseFloat(categoryScores.getProperty("type.cv3.score"))); // cv3 // association
        defaultOntologyScore.put(categoryScores.getProperty("type.cv4.id"), Float.parseFloat(categoryScores.getProperty("type.cv4.score"))); // cv4 // physical association
        defaultOntologyScore.put(categoryScores.getProperty("type.cv5.id"), Float.parseFloat(categoryScores.getProperty("type.cv5.score"))); // cv5 // direct interaction
        /* Ontology terms not present in OLS will be consider null and classify as "unknown" terms */
        defaultOntologyScore.put(categoryScores.getProperty("type.cv6.id"), Float.parseFloat(categoryScores.getProperty("type.cv6.score"))); // cv6 // unknown
